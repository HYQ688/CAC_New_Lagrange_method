function [phi, u, v, w, p, Q, eta_1, eta_2] = CAC_Vesicle_with_NS_3D_newLM_p_SAV_1st(pde,domain,Nx,Ny,Nz,time,option)
...
% Initial energy
if 1 == option.energyflag
    calculate_energy1(out1,out2,t,phi0,u0,v0,w0,Q0);
end
...
%         rhs = rhs./M;
    ...
    G31_u = u0./dt-diff_x(p0); G31_v = v0./dt-diff_y(p0); G31_w = w0./dt-diff_z(p0);
    ...
    Q_2 = nu_21/(1-nu_22);
    nu_31 = get_nu(utilde_31,vtilde_31,wtilde_31,mu_31,phi_star,mu_star,u_star,v_star,w_star) + Q0;
...
%     eta = 1;
    ...
    if 1 == option.energyflag
        calculate_energy1(out1,out2,t,phi0,u,v,w,Q);
    end
...
function [] = calculate_energy1(out1,out2,t,phi,u,v,w,Q)
...
global hx hy hz
    ...
    right1 = eta_1.*fftn(delta_W(phi0).*(eta_1.*phi_1+eta_2.*phi_2+phi_3-phi0));
    right2 = eta_2.*fftn(delta_A(phi0).*(eta_1.*phi_1+eta_2.*phi_2+phi_3-phi0));
