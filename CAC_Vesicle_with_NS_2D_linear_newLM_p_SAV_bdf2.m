function [phi, U, u, v, p, Q, eta2] = CAC_Vesicle_with_NS_2D_linear_newLM_p_SAV_bdf2(pde,domain,Nx,Ny,time,option)
...
[phi1,U1,u1,v1,p1,Q1,~] = CAC_Vesicle_with_NS_2D_linear_newLM_p_SAV_1st(pde,domain,Nx,Ny,time1,option);
...
for nt = 2:nplot
    ...
    g3 = delta_A(phi_star) - 1./(Lx.*Ly).*fun_inner(1,delta_A(phi_star));
    g4 = g2;
   ...
   %step 4    
    if isfield(pde,'rhs2') && isfield(pde,'exactu')
        rhs2 = pde.rhs2(xx,yy,t,eta_2);
        rhs3 = pde.rhs3(xx,yy,t,eta_2);
    else
        rhs2 = 0;
        rhs3 = 0;
    end

    G1_u = rhs2 + (4*u1-u0)./dt/2 - diff_x(p1);  G1_v = rhs3 + (4*v1-v0)./dt/2 - diff_y(p1);
