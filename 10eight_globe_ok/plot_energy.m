...
plot(energy1(tmp:1:end,1),energy1(tmp:1:end,2),'b-','LineWidth',2.5);
hold on
% plot(energy1(tmp:1:end,1),energy1(tmp:1:end,4),'r:','LineWidth',2.5);
% hold on
plot(energy1(tmp:1:end,1),energy2(tmp:1:end,2),'r--','LineWidth',2.5);
% hold on
% plot(energy2(tmp:1:end,1),energy2(tmp:1:end,3),':','LineWidth',2.5);
...
% legend('First scheme','Second scheme','Fontsize',20,'interpreter','latex','box','off');
...
% hold on
% plot(mass3(tmp:1:end,1),abs((mass3(tmp:1:end,3)-mass3(1,3))./mass3(1,3)),'-.','LineWidth',2.5,'Color',fillcolor1);
...
% xlabel('Time','Fontsize',20);ylabel('Ratio of Surface Area Difference','Fontsize',17,'interpreter','latex');
% xlim([0,2])
% ylim([0,1e-4])
...
% % set(get(gca,'YLabel'),'FontSize',figure_FontSize);
% % set(findobj('FontSize',10),'FontSize',figure_FontSize); %这4句是将字体大小改为8号字，在小图里很清晰
% legend('First scheme','Second scheme','First scheme','Second scheme','Fontsize',15,'interpreter','latex','box','off');
% 
% figname2 = ['C:\Users\heyan\Desktop\some files\figs\deformation\3deight_globes\',scheme1,'_and_',scheme2,'_and_',scheme3,'_stability_surface&energy_2D','.png'];
