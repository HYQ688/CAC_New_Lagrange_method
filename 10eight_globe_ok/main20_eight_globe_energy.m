...
para.name = [scheme,'_ex02_Vesicles_data_dt'];
pde = ex02_Vesicles_data(para);
...
elseif 1 == strcmp(scheme,'linear_2cn')
    solver_fun = @CAC_Vesicle_with_NS_3D_linear_newLM_p_SAV_2cn;
...
elseif 1 == strcmp(scheme,'MSAV_bdf2')
    solver_fun = @CAC_Vesicle_NS_3D_BDF_SAV;
end
...
lineType = {'-','-','-','-' ,'-','b-','-','-','-','-k','-b'};
for k = 1:maxIt
    energy=load([pde.name,num2str(dt_array(k)),'_S14_energy.txt']);
    tmp = 1;
...
figname1 = ['C:\Users\heyan\Desktop\some files\figs\deformation\3Deight_globes\',para.name,'_Original_energy','.png'];
...
lineType = {'-','-','-','-' ,'-','b-','-','-','-','-k','-b'};
for k = 1:maxIt
    mass=load([pde.name,num2str(dt_array(k)),'_S14_mass_data.txt']);
...
figname2 = ['C:\Users\heyan\Desktop\some files\figs\deformation\3Deight_globes\',scheme,'_2D_surface','.png'];
